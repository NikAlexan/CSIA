...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit',
        href: `/notifications/schedule/${props.notificationSchedule.id}/edit`,
    },
];
...
setValues({
    type: props.notificationSchedule.type,
    notify_at: props.notificationSchedule.notify_at
        ? props.notificationSchedule.notify_at.substring(0, 5) // Extract HH:MM from HH:MM:SS
        : '08:00',
    message: props.notificationSchedule.message || '',
    enabled: props.notificationSchedule.enabled
});

const onSubmit = handleSubmit(async (values) => {
    try {
        router.patch(`/notifications/schedule/${props.notificationSchedule.id}`, {
            ...values,
...
<template>
    <Head :title="`Edit Notification Schedule`" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-6 max-w-2xl mx-auto p-6">
            <h1 class="text-2xl font-bold">Edit Notification Schedule</h1>
                ...
                <div>
                    ...
                    >
                        ...
                        <option
                            ...
                            :key="type.value"
                            :value="type.value"
                            :selected="type.value === notificationSchedule.type"
                        >
                            {{ type.name }}
                        </option>
                    </Field>
